...
```bash
python3 -m src.genre_extractor --extract-from-novel gangster-hk-1983-v2 \
...
- 格式：纯文本 UTF-8（`.txt`）
- 章节标记：支持 `第N章` / `Chapter N` / `一、二、` / `I./II.` / `1./2.` / `===` 六种（见 `src/genre_extractor/chapter_detector.py`）
...
3. `python3 -m src.genre_extractor --extract-from-novel <new-genre-id> --sources novels/<name>.txt --dry-run` 先 dry-run 确认章节能被正确识别
