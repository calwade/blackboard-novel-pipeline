...
| `state/characters-cast.yaml` | 运行时演员表（每章累积），CharacterGuard 双档对接的"先例库"。仅当 setting.yaml.cast_tracking_enabled=true 时存在 | CastUpdater（每章末覆盖式更新） |
...
| Evaluator | chNNN.md + landmines + iron-laws（core + extra）+ characters + timeline + **current_status_card.md** + **pending_hooks.md**（两者可选，缺失时跳过） | verdict.json + issues.jsonl | 0.0 | 对抗人设，默认拒稿，JSON rubric + skeleton detector；交叉验证"反派信息越界"与伏笔回收一致性 |
...
| **CastUpdater** | chNNN.md + characters.yaml（基线）+ 上一版 characters-cast.yaml | **characters-cast.yaml**（覆盖式） | 0.2 | 运行时累积演员表，每章末更新；新角色 in_baseline=false，基线角色 traits 只能加不能改；与 CharacterGuard 双档对接 |
