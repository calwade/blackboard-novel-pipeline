...
class Planner(BaseAgent):
    ...
    def _build_prompts(self, bb: Blackboard, *, chapter: int, **_):
        ...
        system = (
            ...
            "5b. **章节标题（title 字段）必须丰富、有信息量**：\n"
            "    - 格式：`第 N 章 · <章名>`，章名 4-12 字，体现本章核心戏剧动作或意象\n"
            "    - 不允许：单纯输出 `第 N 章` 占位、用『的故事』『之事』空洞后缀、剧透关键转折\n"
            "    - 章名应该让读者扫一眼章节列表就能感受到剧情推进与张力变化\n"
            "    - 例：✅ `第 26 章 · 锅炉房裂嘴` ✅ `第 9 章 · 北京的电话` ❌ `第 9 章` ❌ `第 9 章 · 一个艰难的选择`\n"
            "    - 即使大纲条目的 title 是『第 N 章』占位，你也必须基于本章场景/冲突/钩子重新起一个有内容的章名。\n"
